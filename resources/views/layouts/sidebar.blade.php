<aside class="w-64 bg-gray-900 text-white min-h-screen p-4 flex flex-col justify-between">
    ...
    <div class="border-t border-gray-800 pt-4">
        ...
        <a href="{{ route('profile.edit') }}"
            class="w-full flex items-center px-4 py-2 text-blue-300 hover:bg-gray-800 hover:text-white rounded-lg transition-colors text-sm">
            Profil Saya
        </a>
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full flex items-center px-4 py-2 text-red-400 hover:bg-gray-800 hover:text-red-300 rounded-lg transition-colors text-sm">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
