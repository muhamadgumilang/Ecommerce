        ...
        <header class="bg-white border-b border-slate-200 sticky top-0 z-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    @auth
                        <a href="{{ url('/dashboard') }}" class="px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-xl hover:bg-blue-700 transition">Dashboard</a>
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-medium text-slate-600 hover:text-blue-600 transition">Masuk</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}" class="px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-xl hover:bg-blue-700 transition">Daftar</a>
                        @endif
                    @endauth
                </div>
            </div>
        ...
        <main class="py-12 flex-1">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div class="text-sm font-medium text-slate-600 bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200">
                ...
                @if($products->isEmpty())
                    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
                        <div class="text-slate-400 text-base font-medium">Belum ada produk yang tersedia di katalog saat ini.</div>
                        <p class="text-slate-400 text-xs mt-1">Silakan kembali lagi nanti.</p>
                    </div>
                @else
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @foreach($products as $item)
                            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition flex flex-col">
                                <!-- Gambar Produk -->
                                ...
                                <div class="p-5 flex-1 flex flex-col justify-between">
                                    <div>
                                        ...
                                        <p class="text-slate-500 text-xs mt-1.5 line-clamp-2">{{ $item->description ?? 'Tidak ada deskripsi.' }}</p>
                                    </div>

                                    <div class="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between">
                                        <span class="text-xs font-semibold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg">Aktif</span>
                                        <a href="#" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl transition shadow-sm shadow-blue-100">
                                            Detail
                                        </a>
                                    </div>
                                </div>
