...
<head>
    ...
    <title>{{ $product->product_name }} - {{ config('app.name', 'Zenthercraft') }}</title>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <nav class="flex items-center text-sm text-slate-500 space-x-2">
                    ...
                    <span>&rsaquo;</span>
                    <span class="text-slate-900 font-medium truncate max-w-xs sm:max-w-md">{{ $product->product_name }}</span>
                </nav>

                <a href="{{ route('catalog.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-800 transition">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-6 sm:p-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 items-start">
                    ...
                    <!-- Left: Product Image -->
                    <div class="space-y-4">
                        <div class="w-full aspect-square rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center relative group shadow-inner">
                            @if ($product->image)
                            ...
                            @if ($product->category)
                                <span class="absolute top-4 left-4 bg-white/90 backdrop-blur-md text-blue-600 font-bold text-xs px-3 py-1.5 rounded-full shadow-sm border border-slate-100">
                    ...
                    <div class="flex flex-col justify-between space-y-6">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <span class="inline-flex items-center gap-1 text-xs font-semibold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
                            ...
                            <div class="mt-4 pb-5 border-b border-slate-100">
                                <span class="text-xs text-slate-400 uppercase tracking-wider font-semibold block mb-1">Harga Produk</span>
                            ...
                            <div class="mt-5 space-y-2">
                                <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Deskripsi Produk</h3>
                            ...
                            <!-- Stock Information -->
                            <div class="mt-5 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between text-xs">
                                <span class="text-slate-500 font-medium">Ketersediaan Stok:</span>
                                @if ($product->stock > 5)
                                    <span class="font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200">
                                ...
                                @else
                                    <span class="font-bold text-rose-600 bg-rose-50 px-2.5 py-1 rounded-lg border border-rose-200">
                                        Stok Habis
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="border-t border-slate-100 pt-6">
                            @if ($product->stock > 0)
                                @auth
                                    <form method="POST" action="{{ route('cart.add') }}" class="space-y-4">
                                        ...
                                        <div>
                                            <label for="quantity" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2">
                                                Tentukan Jumlah
                                            </label>
                                            <div class="flex items-center gap-3">
                                                ...
                                                <span class="text-xs text-slate-400">Maks. {{ $product->stock }} unit</span>
                                            </div>
                                        </div>
                                ...
                                @else
                                    <div class="bg-blue-50 border border-blue-100 rounded-2xl p-5 text-center space-y-3">
                                        <p class="text-sm text-blue-900 font-medium">Silakan masuk ke akun Anda untuk memesan produk ini.</p>
                            ...
                            @else
                                <div class="p-4 bg-rose-50 border border-rose-100 rounded-2xl text-center">
                                    <p class="text-sm font-semibold text-rose-700">Mohon maaf, stok produk ini saat ini sedang habis.</p>
                                    <p class="text-xs text-rose-500 mt-1">Silakan cek kembali di lain waktu atau cari produk serupa di katalog kami.</p>
                                </div>
                            @endif
                        </div>

                        <!-- Assurance Icons -->
                        <div class="grid grid-cols-3 gap-2 pt-4 border-t border-slate-100 text-center text-slate-500 text-[11px]">
                            <div class="p-2">
