...
<head>
    ...
    <title>Pesanan Saya - {{ config('app.name', 'E-Commerce') }}</title>
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Pantau status pengiriman dan riwayat pesanan Anda di sini.
                    </p>
                </div>

                <a href="{{ route('catalog.index') }}"
                    class="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 shadow-sm shadow-blue-500/20 transition self-start sm:self-auto">
            ...
            <div class="space-y-4">
                    ...
                    <div
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-4">
                            <div class="flex items-center gap-3">
                                ...
                                <span
                                    class="text-xs text-slate-500">{{ ($order->order_date ?? $order->created_at)->translatedFormat('d F Y, H:i') }}
                                    WIB</span>
                            </div>

                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border {{ $statusStyles[$order->order_status] ?? 'bg-slate-100 text-slate-700' }}">
                        ...
                        <div class="py-4 space-y-3">
                            @foreach ($order->orderDetails->take(2) as $detail)
                                <div class="flex items-center gap-3.5">
                                    <div
                                        class="w-14 h-14 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-slate-500 mt-0.5">
                                            {{ $detail->quantity }} barang &times; Rp
                        ...
                        <div
                            class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-slate-100 pt-4">
                            <div>
                ...
                @empty
                    <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
                        ...
                        <p class="text-sm text-slate-500 mt-1">Anda belum memiliki riwayat pesanan apa pun saat ini.
                        </p>
                        <a href="{{ route('catalog.index') }}"
