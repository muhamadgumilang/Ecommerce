...
<head>
    ...
    <title>Detail Pesanan #{{ $order->order_id }} - {{ config('app.name', 'Zenthercraft') }}</title>
    ...
    <main class="py-8 sm:py-10 flex-1">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('orders.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-800 transition">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-7 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                    <div>
                        <div class="flex items-center gap-3">
                            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Pesanan #{{ $order->order_id }}</h1>
                            @php
                                $statusStyles = [
                                    'Pending Payment' => 'bg-amber-50 text-amber-700 border-amber-200 ring-amber-500/20',
                                ...
                                $currentStyle = $statusStyles[$order->order_status] ?? 'bg-slate-50 text-slate-700 border-slate-200';
                                $currentLabel = $statusLabels[$order->order_status] ?? $order->order_status;
                            @endphp
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ring-1 {{ $currentStyle }}">
                                {{ $currentLabel }}
                            </span>
                        </div>
                        <p class="text-sm text-slate-500 mt-1">
                            Dipesan pada: <span class="font-medium text-slate-700">{{ ($order->order_date ?? $order->created_at)->translatedFormat('d F Y, H:i') }} WIB</span>
                        </p>
                    </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach ($steps as $index => $step)
                                ...
                                <div class="flex items-start gap-3">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-colors {{ $isPassed ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'bg-slate-100 text-slate-400 border border-slate-200' }}">
                                    ...
                                    <div class="min-w-0">
                                        <p class="text-xs font-semibold {{ $isCurrent ? 'text-blue-600' : ($isPassed ? 'text-slate-800' : 'text-slate-400') }}">
                                            {{ $step['label'] }}
                                        </p>
                ...
                @else
                    <div class="mt-4 p-4 rounded-xl bg-rose-50 border border-rose-100 flex items-center gap-3 text-sm text-rose-700">
                        <svg class="w-5 h-5 shrink-0 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span>Pesanan ini telah dibatalkan. Jika Anda memiliki pertanyaan atau kendala, silakan hubungi admin.</span>
                    </div>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="divide-y divide-slate-100">
                            @forelse ($order->orderDetails as $detail)
                                <div class="py-4 first:pt-0 last:pb-0 flex items-center gap-4">
                                    <!-- Product Thumbnail -->
                                    <div class="w-20 h-20 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-semibold text-slate-900 text-base truncate">
                                            @if ($detail->product)
                                                <a href="{{ route('products.show', $detail->product) }}" class="hover:text-blue-600 transition">
                                        ...
                                        @if ($detail->product?->category)
                                            <p class="text-xs text-slate-400 mt-0.5">Kategori: {{ $detail->product->category->category_name }}</p>
                                        @endif
                                        <div class="flex items-center gap-2 mt-2 text-sm text-slate-500">
                                            <span>Rp {{ number_format($detail->product?->price ?? ($detail->subtotal / max(1, $detail->quantity)), 0, ',', '.') }}</span>
                                            <span>&times;</span>
                                            <span class="px-2 py-0.5 bg-slate-100 rounded text-xs font-medium text-slate-700">{{ $detail->quantity }} barang</span>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        <h2 class="font-bold text-slate-900 text-lg flex items-center gap-2 border-b border-slate-100 pb-4 mb-4">
                            <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
                            <div>
                                ...
                                <p class="font-medium text-slate-900 mt-1">{{ $order->customer?->name ?? Auth::user()->name }}</p>
                                <p class="text-xs text-slate-500 mt-0.5">{{ $order->customer?->email ?? Auth::user()->email }}</p>
                            </div>

                            <div>
                                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Metode Kurir</p>
                                <div class="flex items-center gap-2 mt-1">
                                    <span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>
                                    <span class="font-medium text-slate-900">{{ $order->checkout?->courier ?? 'Pengiriman Standar' }}</span>
                                </div>
                            </div>

                            <div class="sm:col-span-2">
                                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Alamat Tujuan</p>
                                <p class="text-slate-800 bg-slate-50 p-3.5 rounded-xl border border-slate-100 mt-1.5 leading-relaxed">
                            ...
                            @if ($order->checkout?->notes)
                                <div class="sm:col-span-2">
                                    <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Catatan Pesanan</p>
                                    <p class="text-slate-600 italic bg-amber-50/50 p-3 rounded-xl border border-amber-100 mt-1.5 text-xs">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                            <div>
                                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Metode Pembayaran</p>
                                <p class="font-medium text-slate-900 mt-1">{{ $order->payment?->payment_method ?? 'Belum Dipilih' }}</p>
                            </div>

                            <div>
                                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Status Pembayaran</p>
                                <div class="mt-1">
                                    @if ($order->payment)
                            ...
                            <div>
                                <p class="text-xs text-slate-400 uppercase tracking-wider font-semibold">Waktu Pembayaran</p>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        ...
                        <div class="mt-6 space-y-3">
                            ...
                            @elseif ($order->payment && $order->payment->payment_status === 'Pending')
                                <div class="rounded-xl bg-amber-50 border border-amber-200 p-3.5 text-xs text-amber-800 flex items-start gap-2.5">
                                    <svg class="w-4 h-4 text-amber-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    <span>Bukti pembayaran sedang diverifikasi oleh admin. Kami akan memperbarui status Anda segera.</span>
                                </div>
                            @elseif ($order->payment && $order->payment->payment_status === 'Verified')
                                <div class="rounded-xl bg-emerald-50 border border-emerald-200 p-3.5 text-xs text-emerald-800 flex items-start gap-2.5">
                    ...
                    <div class="bg-gradient-to-br from-blue-50 to-sky-50 rounded-2xl border border-blue-100 p-5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mt-0.5">Jika pesanan bermasalah, silakan hubungi customer service kami.</p>
                            </div>
                        </div>
                    </div>
