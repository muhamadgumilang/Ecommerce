                ...
                <form action="{{ route('seller.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <span class="text-xs text-slate-400 mt-1 block">Biarkan kosong jika tidak ingin mengganti</span>
                            @error('image')
                                <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                            @enderror
                            ...
                            @if (!empty($product->image))
                                <div class="mt-3 flex items-center gap-3">
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->product_name }}" class="w-14 h-14 rounded-lg object-cover border border-slate-200">
