<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-xl text-slate-800 leading-tight">Detail Pesanan #{{ $order->order_id }}
                    </h2>
                    <p class="text-xs text-slate-500 mt-0.5">Dibuat pada
                        {{ ($order->order_date ?? $order->created_at)->translatedFormat('d F Y, H:i') }} WIB</p>
                </div>
            </div>

            <div class="flex items-center gap-2.5">
                <a href="{{ route('admin.orders.edit', $order) }}"
                    class="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-semibold hover:bg-blue-700 shadow-sm shadow-blue-500/20 transition">
                ...
                <a href="{{ route('admin.orders.index') }}"
                    class="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl text-xs font-semibold hover:bg-slate-200 transition">
                    Kembali
                </a>
            </div>
        </div>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (session('success'))
                <div
                    class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-2xl text-sm shadow-sm flex items-center gap-2">
            ...
            <!-- Quick Status Banner -->
            <div
                class="bg-white shadow-sm sm:rounded-2xl border border-slate-100 p-6 flex flex-wrap items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-lg">
                        #{{ $order->order_id }}
                    </div>
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            @endphp
                            <span
                                class="px-3 py-1 rounded-full text-xs font-semibold border {{ $statusStyles[$order->order_status] ?? 'bg-slate-100 text-slate-700' }}">
                                {{ $statusLabels[$order->order_status] ?? $order->order_status }}
                            </span>
                        </div>
                        <p class="text-xs text-slate-500 mt-1">Total Tagihan: <strong
                                class="text-blue-600 font-bold text-sm">Rp
                ...
                <form method="POST" action="{{ route('admin.orders.update', $order) }}"
                    ...
                    <select id="quick_status" name="order_status"
                        class="text-xs rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 py-1.5 pl-3 pr-8">
                    ...
                    <button type="submit"
                        class="px-3 py-1.5 bg-slate-800 text-white rounded-xl text-xs font-medium hover:bg-slate-900 transition">
                        Update
                    </button>
                </form>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div class="lg:col-span-2 space-y-6">
                                ...
                                <div class="py-3.5 first:pt-0 last:pb-0 flex items-center gap-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-xs text-slate-500 mt-1">
                                            Rp
                                            {{ number_format($detail->product?->price ?? $detail->subtotal / max(1, $detail->quantity), 0, ',', '.') }}
                                            &times; {{ $detail->quantity }} item
                                        </p>
                                    </div>
                        ...
                        <div class="border-t border-slate-100 pt-4 mt-4 space-y-2 text-sm">
                            <div class="flex justify-between text-slate-500 text-xs">
                                ...
                                <span class="font-medium text-slate-800">Rp
                                    {{ number_format($order->orderDetails->sum('subtotal'), 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between text-slate-500 text-xs">
                            ...
                            <div
                                class="flex justify-between font-bold text-slate-900 text-base pt-2 border-t border-slate-100">
                    ...
                    <div class="bg-white shadow-sm sm:rounded-2xl border border-slate-100 p-6">
                        <h3
                            class="font-bold text-slate-800 text-base flex items-center gap-2 border-b border-slate-100 pb-4 mb-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p class="font-semibold text-slate-800 mt-1">
                                    {{ $order->checkout?->courier ?? 'Pengiriman standar' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-slate-400 font-medium uppercase">Biaya Kirim</p>
                                <p class="font-semibold text-slate-800 mt-1">Rp
                                    {{ number_format($order->checkout?->shipping_fee ?? 0, 0, ',', '.') }}</p>
                            </div>
                            <div class="sm:col-span-2">
                                <p class="text-xs text-slate-400 font-medium uppercase">Alamat Pengiriman</p>
                                <p
                                    class="bg-slate-50 p-3 rounded-xl border border-slate-100 text-slate-700 mt-1.5 leading-relaxed">
                            ...
                            @if ($order->checkout?->notes)
                                <div class="sm:col-span-2">
                                    ...
                                    <p
                                        class="bg-amber-50/60 p-3 rounded-xl border border-amber-100 text-amber-800 italic text-xs mt-1.5">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white shadow-sm sm:rounded-2xl border border-slate-100 p-6">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <div
                                class="w-11 h-11 rounded-full bg-gradient-to-br from-blue-500 to-sky-400 flex items-center justify-center font-bold text-white shadow-sm">
                        ...
                        <div class="space-y-2 text-xs border-t border-slate-100 pt-3 text-slate-600">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span
                                    class="font-medium text-slate-700">{{ $order->customer?->role ?? 'Customer' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white shadow-sm sm:rounded-2xl border border-slate-100 p-6">
                        ...
                        <div class="space-y-3 text-sm">
                            <div>
                                ...
                                <p class="font-semibold text-slate-800 mt-1">
                                    {{ $order->payment?->payment_method ?? 'Belum memilih metode' }}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-1">
                                    @if ($order->payment)
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border {{ $payStyles[$order->payment->payment_status] ?? 'bg-slate-100 text-slate-700' }}">
