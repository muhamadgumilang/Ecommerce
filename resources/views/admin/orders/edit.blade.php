<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800">Edit Status Pesanan #{{ $order->order_id }}</h2><a
                href="{{ route('admin.orders.index') }}" class="text-blue-600 hover:underline">Kembali</a>
        </div>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-2xl border border-slate-100 p-6">
                ...
                <form method="POST" action="{{ route('admin.orders.update', $order) }}" class="space-y-4">
                    @csrf @method('PUT')
                    <div><label for="order_status" class="block text-sm font-medium text-slate-700">Status
                            Pesanan</label><select id="order_status" name="order_status"
