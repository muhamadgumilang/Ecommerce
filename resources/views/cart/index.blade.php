    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <nav class="flex items-center gap-4 sm:gap-6">
                ...
                <a href="{{ route('cart.index') }}" class="text-xl leading-none text-blue-600 transition" title="Keranjang" aria-label="Keranjang">
                    &#128722;
                </a>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Periksa kembali barang pilihan Anda dan lanjutkan ke proses pemesanan.</p>
                </div>

                @if ($cart && $cart->cartItems->isNotEmpty())
                    <div class="text-xs font-medium text-slate-600 bg-white px-3.5 py-2 rounded-xl shadow-sm border border-slate-200 self-start sm:self-auto">
                        Total Produk di Keranjang: <span class="text-blue-600 font-bold">{{ $cart->cartItems->count() }} item</span> ({{ $cart->cartItems->sum('quantity') }} barang)
                    </div>
                @endif
            </div>
            ...
            @if (!$cart || $cart->cartItems->isEmpty())
                <!-- Empty Cart State -->
                <div class="bg-white rounded-2xl border border-slate-200 p-12 sm:p-16 text-center shadow-sm max-w-2xl mx-auto">
                    <div class="w-20 h-20 mx-auto rounded-full bg-blue-50 text-blue-500 flex items-center justify-center text-4xl mb-5">
                        &#128722;
                    </div>
                    ...
                    <p class="text-sm text-slate-500 mt-2 max-w-md mx-auto">
                        Anda belum menambahkan produk apa pun ke keranjang. Temukan berbagai penawaran dan produk menarik di katalog kami.
                    </p>
            ...
            @else
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        <div class="bg-blue-50/70 border border-blue-100 rounded-2xl p-4 text-xs text-blue-800 flex items-center gap-3">
                            <svg class="w-5 h-5 text-blue-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <span>Pilih produk yang ingin di-checkout, atau gunakan opsi <strong>Checkout Semua</strong> di ringkasan belanja.</span>
                        </div>
                        ...
                        @foreach ($cart->cartItems as $index => $item)
                            <div class="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-sm hover:border-blue-300 transition flex flex-col sm:flex-row sm:items-center gap-4 relative group">
                                ...
                                <div class="flex items-center gap-3 shrink-0">
                                    ...
                                    <!-- Product Thumbnail -->
                                    <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                                ...
                                <div class="min-w-0 flex-1">
                                    <h2 class="font-semibold text-slate-900 text-base leading-snug">
                                        <a href="{{ route('products.show', $item->product) }}" class="hover:text-blue-600 transition">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        @if ($item->product->category)
                                            <span class="text-[11px] font-medium bg-slate-100 text-slate-600 px-2 py-0.5 rounded">
                                                {{ $item->product->category->category_name }}
                                            </span>
                                        @endif
                                        <span class="text-xs text-slate-500">
                                            Harga: <strong class="text-slate-700">Rp {{ number_format($item->product->price, 0, ',', '.') }}</strong>
                                        </span>
                                    </div>
                                    ...
                                    <div class="mt-3 flex flex-wrap items-center justify-between gap-3">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <!-- Decrement Form -->
                                            <form method="POST" action="{{ route('cart.update', $item->cart_item_id) }}" class="inline">
                                                @csrf
                                                @method('PATCH')
                                ...
                                <div class="self-end sm:self-start pt-1 sm:pt-0">
                                    <form method="POST" action="{{ route('cart.remove', $item->cart_item_id) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini dari keranjang?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <aside class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            ...
                            <div class="mt-4 space-y-3 text-sm">
                                <div>
                                    ...
                                    <p id="selected-product-name" class="font-medium text-slate-800 truncate mt-0.5">-</p>
                                </div>
                                ...
                                <div class="border-t border-slate-200 pt-3 mt-2 flex justify-between items-center text-base font-bold text-slate-900">
                        ...
                        <div class="bg-gradient-to-br from-blue-50 to-sky-50 rounded-2xl border border-blue-100 p-5 space-y-3 text-xs text-slate-600">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-500 mt-0.5">Pembayaran Anda dilindungi sistem verifikasi pesanan yang terpercaya.</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start gap-3 pt-2 border-t border-blue-100">
                                ...
                                <div>
                                    ...
                                    <p class="text-slate-500 mt-0.5">Pesanan langsung dikemas dan dikirim ke alamat tujuan Anda.</p>
                                </div>
                            </div>
                        </div>
