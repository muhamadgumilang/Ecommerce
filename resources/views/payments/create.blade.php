...
<body
    ...
    <main class="py-8 sm:py-10 flex-1">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex items-center justify-between">
                <nav class="flex items-center text-sm text-slate-500 space-x-2">
                    ...
                    <a href="{{ route('orders.show', $order) }}"
                        class="hover:text-blue-600 transition">#{{ $order->order_id }}</a>
                    <span>&rsaquo;</span>
            ...
            <!-- Page Title -->
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">Pembayaran Pesanan
                    #{{ $order->order_id }}</h1>
                <p class="text-sm text-slate-500 mt-1">Pilih saluran pembayaran yang sesuai untuk memproses pesanan
                    Anda.</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-7 shadow-sm">
                        ...
                        <form id="payment-form" method="POST" action="{{ route('payments.store', $order) }}"
                            class="space-y-4">
                            @csrf

                            <!-- Midtrans Sandbox -->
                            <label
                                class="payment-card flex items-start gap-4 p-4 rounded-2xl border-2 border-blue-500 transition cursor-pointer bg-white">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center justify-between gap-2">
                                        <h3 class="font-bold text-slate-900 text-sm">Midtrans Simulator</h3>
                                        <span
                                            class="text-[11px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded">Sandbox</span>
                                    </div>
                                    <p class="text-xs text-slate-500 mt-1">Pilih metode pembayaran dan simulasikan
                                        transaksi melalui popup Midtrans.</p>
                            ...
                            <!-- Option 2: E-Wallet -->
                            <label
                                class="payment-card hidden flex items-start gap-4 p-4 rounded-2xl border-2 border-slate-200 hover:border-blue-500 transition cursor-pointer bg-white">
                                <input type="radio" name="payment_method" value="E-Wallet"
                                    class="mt-1 w-5 h-5 text-blue-600 border-slate-300 focus:ring-blue-500">
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center justify-between gap-2">
                                        <h3 class="font-bold text-slate-900 text-sm">Dompet Digital (E-Wallet / QRIS)
                                        </h3>
                                        <span
                                            class="text-[11px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">Instan</span>
                                    </div>
                                    <p class="text-xs text-slate-500 mt-1">GoPay, OVO, DANA, ShopeePay, LinkAja via
                                        QRIS Nasional.</p>

                                    <div
                                        class="mt-3 p-3 bg-slate-50 border border-slate-100 rounded-xl text-xs text-slate-700">
                                        <p class="text-slate-600">Scan kode QRIS yang tersedia pada struk pembayaran
                                            atau konfirmasi via WhatsApp customer service.</p>
                                    </div>
                                </div>
                            </label>
                            ...
                            <label
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center justify-between gap-2">
                                        <h3 class="font-bold text-slate-900 text-sm">Cash on Delivery (Bayar di Tempat)
                                        </h3>
                                        <span
                                            class="text-[11px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded">Bayar
                                            Nanti</span>
                                    </div>
                                    <p class="text-xs text-slate-500 mt-1">Bayar secara tunai ke kurir saat barang
                                        sampai di alamat rumah Anda.</p>
                                </div>
                            </label>
                ...
                <div class="space-y-6">
                    ...
                    <aside class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                        ...
                        <p class="text-[11px] text-slate-400 text-center mt-4 leading-relaxed">
                            Pembayaran diproses melalui Midtrans Sandbox untuk simulasi transaksi.
                        </p>
                    </aside>
                    ...
                    <div
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 mt-0.5">Seluruh data transaksi dan data pengguna disimpan
                                    secara aman.</p>
                            </div>
                        </div>
                    </div>
